...
const dog = async () => {
    ...
    const dogsSorted = response.data.dogs.sort((a, b) => b.naughty - a.naughty)
    return dogsSorted[0]
};
...
const joke = async () => {
    const response1 = await fetch('jokes', 'question')
    const response2 = await fetch('jokes')
    return { question: response1.joke, answer: response2.answer}
};
